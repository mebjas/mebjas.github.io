  ...
  _createClass(Html5Qrcode, [{
    ...
    value: function start(cameraId, configuration, qrCodeSuccessCallback, qrCodeErrorCallback) {
      ...
      var setupUi = function setupUi(width, height) {
        ...
        if (qrboxSize > height) {
          qrboxSize = height;
          console.log("[Warning] [Html5Qrcode] config.qrboxsize is greater " + "than video height. It's being auto modified to height of " + "the video = ".concat(height));
        }
        ...
        var qrRegion = isShadedBoxEnabled ? _this._getShadedRegionBounds(width, height, qrboxSize) : defaultQrRegion;

        var canvasElement = _this._createCanvasElement(qrRegion.width, qrRegion.height);

        var context = canvasElement.getContext('2d');
        context.canvas.width = qrRegion.width;
        context.canvas.height = qrRegion.height; // Insert the canvas and video
